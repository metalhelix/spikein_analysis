...
plot(log2(spikes_merged$mix_1), log2(spikes_merged$rpkm), col = spikes_merged$color, xlab = "Log2(pM) Mix 1", ylab = "Log2(RPKM Mix 1)")
...
text(12, 10, labels = slope_text)
r2_text <- paste("R2 = ", fit_r_squared, sep = "")
text(12, 9.3, labels = r2_text)
dev.off()

filename <- paste(output_prefix, name, "_", "expected_vs_mix_2", ".png", sep = "")
png(filename, height=500, width=800)
plot(log2(spikes_merged$mix_2), log2(spikes_merged$rpkm), col = spikes_merged$color, xlab = "Log2(pM) Mix 2", ylab = "Log2(RPKM Mix 2)")
